import { getOrderRefundStatusByPhone, getOrderStatusByName, getOrderStatusByPhoneNumber,getOrderRefundStatusByOrderName, cancelOrderByPhone, cancelOrderByOrderName } from "./shopify.js";
const mapExotelRequests = async(req, res) => {
  try {
    const appId = req.query.flow_id.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, '');
    const customerPhone = req.query.From;
    const digitsInserted = req.query.digits.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, ''); 
    if (!appId) {
    ...
    const {text,whatsapp} = mapRequestToPlainText(status,data.order,data.statusText);
    return {
        text: text,
        whatsapp,
        order: data.order
...
const mapRequestToPlainText = (status,order,statusText) =>{
    ...
    switch(status){
      ...
      break;
      case "cancelled":
        text = `Your order was cancelled successfully on ${new Date(order.cancelledAt).toDateString()}. Prepaid orders are refunded automatically in 5 to 7 working days on source account.`
      break;
      case "returned":
      ...
      break;
      case "in-transit":
        text = "Your order is in transit and will be delivered to you shortly. Kindly check your whatsapp or email for the tracking link."
      break;
      case "packed":
        text = "Your latest order is packed safely and will be delivered to you in 2 to 5 working days"
      break;
      case "placed":
        text = "Your latest order is successfully confirmed and will be delivered to you in 2 to 5 working days"
      break;
      case "order_cod_refund_not_eligible":
        text = "Refund for your latest order is not eligible as you placed a cash on delivery order.To know about our refund policy, you can check the message on whatsapp that will be sent to you shortly."
      ...
      case "order_already_cancelled" :
        text = order.paymentGatewayNames.indexOf("cash_on_delivery") != -1 ? `Your order is already cancelled on ${new Date(order.cancelledAt).toDateString()}` : `Your order placed on ${new Date(order.createdAt).toDateString()} is already cancelled. Your refund of amount ${order.refunds[0].totalRefunded.amount}  is initiated and will be credited in your source account in 5 to 7 working days from the date of cancellation.`
      break;
      ...
      case 'order_cancelled' :
        text = order.paymentGatewayNames.indexOf("cash_on_delivery") != -1 ? `Your cash on delivery order placed on ${new Date().toDateString()} is cancelled successfully.` : `Your order placed on ${new Date(order.createdAt)} is cancelled successfully.Your refund of amount ${new Date(order.currentTotalPriceSet.shopMoney.amount)} is initiated and will be credited within 5-7 working days in your account from which the transaction was made.`; 
    };
